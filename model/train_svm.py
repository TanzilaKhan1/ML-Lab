"""SVM classifier on HOG features.

Pipeline: PNG → grayscale → resize 128x128 → HOG → StandardScaler → PCA → SVM.
Stratified 80/20 split. GridSearchCV over kernel/C/gamma. Saves joblib.
"""
...
def main() -> None:
    ...
    joblib.dump(model, OUT_DIR / "svm_model.joblib")
    ...
    (OUT_DIR / "report.txt").write_text(
        ...
        f"test acc: {acc:.4f}\n\n"
        f"confusion matrix:\n{cm}\n\n{report}\n")
    (OUT_DIR / "best_params.json").write_text(json.dumps(search.best_params_, indent=2))
    print(f"\nsaved: {OUT_DIR}/{{svm_model.joblib,confusion_matrix.png,report.txt,best_params.json}}")
