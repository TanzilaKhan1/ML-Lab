...
\begin{itemize}
    ...
    \item An evaluation using standard classification metrics, including accuracy, unsafe-class recall, confusion matrices, ROC and precision--recall curves, and AUC, together with a train/validation/test generalization analysis.
\end{itemize}
...
AUC values are reported in Table~\ref{tab:test_results}. Figure~\ref{fig:paper_roc_test} shows held-out ROC curves for all reported models, while Fig.~\ref{fig:paper_pr_curves} shows the corresponding precision--recall curves and Fig.~\ref{fig:per_class_recall} reports safe- and unsafe-class recall at the balanced operating point.
