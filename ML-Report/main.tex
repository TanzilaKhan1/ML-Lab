...
\IfFileExists{arxiv.sty}{\usepackage{arxiv}}{}
\usepackage{graphicx}
...
% Keep bibliography links compact and clickable rather than exposing long raw URLs.
...
\title{Detection of Unsafe Hanging Travel in Public Transport Using Machine Learning}

\author{Tazkia Malik, Sadek Hossain Asif, Tanzila Khan\\
Taif Ahmed Turjo, Amio Rashid, Mahmud Hasan Walid\\
...
\hypersetup{
  pdftitle={Detection of Unsafe Hanging Travel in Public Transport Using Machine Learning},
  pdfauthor={Tazkia Malik, Sadek Hossain Asif, Tanzila Khan, Taif Ahmed Turjo, Amio Rashid, Mahmud Hasan Walid},
  pdfsubject={Image-based binary classification of unsafe hanging travel in public transport},
  pdfkeywords={unsafe public transport, hanging travel, computer vision, image classification, Dhaka}
}
...
\begin{abstract}
Unsafe door-hanging and overcrowded travel are persistent safety risks in Dhaka's public transport system, particularly in buses and legunas operating under high passenger demand. Existing intelligent transportation research has addressed vehicle detection, passenger counting, and general crowd analysis, but localized automated detection of unsafe hanging travel remains limited. This paper presents an image-based machine-learning framework for classifying public transport scenes into safe/normal and unsafe/risky conditions. A custom bus and leguna image dataset is prepared through cleaning, annotation, preprocessing, and augmentation. For classical machine-learning baselines, Histogram of Oriented Gradients (HOG) descriptors are combined with Logistic Regression, Gaussian Naive Bayes, and a Support Vector Machine (SVM) using a radial basis function kernel. Deep models including a CNN trained from scratch, ResNet18, EfficientNet-B0, ResNet50, ConvNeXt-Tiny, and an ensemble are also evaluated to compare learned visual features with handcrafted HOG representations. The retrained dataset contains 523 annotation-labeled images, and augmentation uses train-only augmentation that avoids augmentation leakage. On the untouched 79-image test set, the ResNet50--ConvNeXt ensemble reaches 94.7\% unsafe recall, 86.1\% accuracy, and 0.970 ROC-AUC at the balanced operating point, while ResNet50 attains the highest single-model accuracy of 92.4\%. Because this is intended for safety-oriented monitoring, the system is tuned to prioritize unsafe-class recall while retaining acceptable discrimination. The current run yields promising unsafe-hanging-travel classification performance, while the small test set limits the precision of the estimate.
\end{abstract}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.98\linewidth,height=0.32\textheight,keepaspectratio]{figures/fig01_dataset_overview.png}
...
Representative examples of the vehicle categories are shown in Figure~\ref{fig:leguna_samples} and Figure~\ref{fig:bus_samples}.
...
\begin{figure}[!htbp]
    \centering
    \subfloat[Unsafe/risky annotation.]{\includegraphics[width=0.35\linewidth]{annotation_1.jpeg}}
    \hfill
    \subfloat[Safe/normal annotation.]{\includegraphics[width=0.35\linewidth]{annotation_2.jpeg}}
    \caption{Representative annotation examples.}
    \label{fig:annotation_samples}
\end{figure}

\subsection{Data Augmentation}
The source images were EXIF-orientation-corrected, converted to RGB, resized so the shorter side was 512 pixels, center-cropped to $512\times512$, and written as PNG before splitting. The repository stores raw images and annotations in Cloudflare R2; the available records do not document the original acquisition procedure or collection period. Only the 365 training originals were augmented offline, producing 2,365 copies and 2,730 training images with 1,330 safe and 1,400 unsafe samples; validation and test remain 79 unaugmented originals each (41 safe and 38 unsafe). Offline augmentation used the implemented Albumentations pipeline: horizontal flip ($p=0.50$); one of affine transformation (scale 0.90--1.12, translation up to 8\%, rotation $\pm15^\circ$, shear $\pm8^\circ$), perspective distortion (scale 0.02--0.08), or random-resized crop (scale 0.78--1.00, aspect ratio 0.8--1.25) with $p=0.90$; brightness/contrast, gamma (70--140), or color jitter with $p=0.85$; hue--saturation--value, RGB shift, or CLAHE with $p=0.55$; sharpening or embossing ($p=0.20$); blur ($p=0.30$); noise ($p=0.30$); compression or downscaling ($p=0.25$); shadow, fog, or sun flare ($p=0.15$); coarse dropout ($p=0.30$); and grayscale ($p=0.05$). Thus, offline transforms are applied after splitting and only to training originals. The public implementation gives the exhaustive transform parameterization.

During deep-model training, the online pipeline applies random-resized crop (scale 0.8--1.0, ratio 0.8--1.25), horizontal flip, $\pm10^\circ$ rotation, RandAugment (two operations, magnitude 7), ColorJitter (0.25, 0.25, 0.25, 0.05), tensor conversion, ImageNet normalization, and random erasing ($p=0.25$, scale 0.02--0.12). Evaluation preprocessing uses resize to 1.15 times the model input followed by center crop and ImageNet normalization. Test-time augmentation averages the original and horizontally flipped evaluation predictions for deep models. Weighted random sampling and class-weighted cross-entropy or focal loss are training controls, not image transforms. These transformations simulate common roadside imaging variation while preserving the class label \cite{shorten2019augmentation}.

All images were standardized before model-specific processing. The standardized preprocessing and model input settings are summarized in Table~\ref{tab:model_config}. The dataset is available through \href{https://data.mendeley.com/datasets/24n4zpdt69/1}{Mendeley Data}.
...
\begin{enumerate}
    ...
    \item \textbf{Four-way stratification:} Split the originals 70/15/15 over vehicle type and class, preserving bus-safe, bus-unsafe, leguna-safe, and leguna-unsafe samples in validation and test partitions. The code does not perform scene-ID, event, or near-duplicate grouping, so this stratification alone does not establish independence at those levels.
    \item \textbf{Train-only augmentation:} Apply offline augmentation only to the 365 training originals, avoiding augmentation leakage and producing 2,730 training images. Keep the 79-image validation and 79-image test partitions unaugmented.
    ...
    \item \textbf{Model training:} Train Logistic Regression, Gaussian Naive Bayes, SVM, and the scratch CNN as baselines. For each pretrained backbone, train the new head alone for the configured warm-up, then unfreeze the entire backbone and continue fine-tuning with discriminative learning rates. Use weighted sampling and the configured weighted cross-entropy or focal loss.
    \item \textbf{Validation and ensembling:} Tune balanced, high-recall (at least 95\% validation unsafe recall), and max-recall (100\% validation unsafe recall) thresholds on validation predictions. The final model-selection procedure fixes the final ensemble as the probability average of ResNet50 and ConvNeXt-Tiny; this choice and all thresholds are made before test evaluation.
...
The Support Vector Machine (SVM) identifies a decision boundary that maximizes the margin between the negative and positive classes \cite{cortes1995svm}. To capture non-linear patterns in the HOG feature space, the radial basis function (RBF) kernel is used to quantify the similarity between pairs of feature vectors,
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.98\linewidth,height=0.45\textheight,keepaspectratio]{arch_resnet.png}
    \caption{Two-phase transfer-learning scheme, illustrated with ResNet18. The ImageNet-pretrained backbone is frozen during head-only warm-up; after warm-up, all backbone parameters are unfrozen and fine-tuned with a lower learning rate than the new classifier head. ResNet50, EfficientNet-B0, and ConvNeXt-Tiny follow the same procedure at a $224\times224$ input resolution.}
    \label{fig:arch_resnet}
\end{figure}

\section{Experimental Setup}
\subsection{Split Protocol}
The data split followed a four-way stratified protocol over vehicle type and class, producing 70/15/15 training, validation, and test partitions. This ensures that bus-safe, bus-unsafe, leguna-safe, and leguna-unsafe samples occur in both validation and test sets. Augmentation was applied only after splitting and only to training originals. Thresholds were tuned on validation predictions, while the 79-image test set remained untouched until final evaluation. The final model-selection procedure fixes ResNet50 and ConvNeXt-Tiny as ensemble members and derives the balanced, high-recall, and max-recall thresholds from validation predictions before test scoring; the test set was not used for threshold tuning or ensemble construction. The full nine-model test table is reported for transparency.

\subsection{Model Configuration}
The experiments were implemented in Python using scikit-learn for the classical pipelines \cite{pedregosa2011sklearn}, PyTorch for the neural model \cite{paszke2019pytorch}, and scikit-image for image processing \cite{vanderwalt2014skimage}. The training script records the seeds for each candidate run; model and preprocessing settings are consolidated in Table~\ref{tab:model_config}.
...
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.25\linewidth} >{\raggedright\arraybackslash}p{0.17\linewidth} >{\raggedright\arraybackslash}X}
\toprule
...
ResNet18 & RGB $224\times224$ & ImageNet V1; two-phase full-backbone fine-tuning; hflip TTA. \\
EfficientNet-B0 & RGB $224\times224$ & ImageNet V1; two-phase full-backbone fine-tuning; hflip TTA. \\
ResNet50 & RGB $224\times224$ & ImageNet V2; two-phase full-backbone fine-tuning; hflip TTA. \\
ConvNeXt-Tiny & RGB $224\times224$ & ImageNet V1; two-phase full-backbone fine-tuning; hflip TTA. \\
...
Deep-model training & RGB tensors & Common protocol and recorded candidate-run settings are documented below; the checkpoint-selection rule is highest validation balanced accuracy, but the exact winning configuration is not recoverable from the saved artifacts. \\
Evaluation protocol & \shortstack[l]{Train/\\validation/test} & Four-way stratified 70/15/15 split; 365 training originals expanded to 2,730 train images, with 79 unaugmented validation and 79 untouched test images. \\
\bottomrule
\end{tabularx}
\end{table}

The final protocol used ImageNet-pretrained transfer backbones with a head-only warm-up followed by full-backbone fine-tuning, AdamW, cosine annealing, weighted sampling, horizontal-flip TTA, and checkpoint selection by validation balanced accuracy. The training records document candidate configurations, including batch sizes of 24 or 32, 45--70 total epochs, 5 or 6 warm-up epochs, model-specific learning rates and weight decay, dropout, label smoothing, and focal-loss alternatives.
...
\end{split}
\end{equation}
The Matthews correlation coefficient (MCC) measures the correlation between the predicted and true binary labels. By incorporating all four entries of the confusion matrix, it produces a high score only when the classifier performs consistently well across both classes. This makes MCC a particularly informative single-value measure, especially when class distributions are imbalanced \cite{chicco2020mcc}:

\begin{equation}
\mathrm{MCC} = \frac{TP \cdot TN - FP \cdot FN}
{\sqrt{(TP{+}FP)(TP{+}FN)(TN{+}FP)(TN{+}FN)}}. \label{eq:mcc}
\end{equation}
The coefficient ranges from $-1$ to $+1$. A value of $+1$ represents perfect agreement between predictions and true labels, $0$ indicates no correlation, and $-1$ reflects complete disagreement. When the denominator is undefined because one of its factors is zero, the coefficient is conventionally assigned a value of $0$. In addition to these threshold-dependent measures, two threshold-independent metrics are used to assess ranking performance across all possible decision thresholds. ROC-AUC measures the area under the receiver operating characteristic curve, which plots the true positive rate against the false positive rate. It can also be interpreted as the probability that the model assigns a higher score to a randomly selected unsafe image than to a randomly selected safe image. PR-AUC, reported as average precision, summarizes the relationship between precision and recall across different classification thresholds.

\begin{equation}
\mathrm{AP} = \sum_{n} \left(R_n - R_{n-1}\right) P_n, \label{eq:ap}
\end{equation}
where $P_n$ and $R_n$ are the precision and recall at the $n$-th threshold; its no-skill baseline equals the unsafe-class prevalence, which makes it informative for the safety-critical positive class. Because missed unsafe cases have direct safety implications, unsafe-class recall is emphasized alongside overall accuracy, MCC, and AUC. Given the small test set, uncertainty is reported as 95\% Wilson score intervals, and pairwise differences between the strongest models are assessed with McNemar's exact test. For the ensemble, unsafe recall is $36/38=94.7\%$, with a 95\% Wilson interval of $[82.7\%,98.5\%]$.


\section{Results}
\subsection{Quantitative Comparison}
Table~\ref{tab:test_results} reports balanced operating-point performance on the untouched 79-image test set. The ResNet50--ConvNeXt ensemble reaches 0.861 accuracy, 0.947 unsafe recall, and 0.970 ROC-AUC. ResNet50 attains the highest single-model accuracy (0.924) and MCC (0.848), while EfficientNet-B0 attains the highest unsafe recall (0.974) and ROC-AUC (0.979). All nine models are compared in Figure~\ref{fig:paper_model_comparison}.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.98\linewidth,height=0.34\textheight,keepaspectratio]{figures/fig02_model_comparison.png}
...
AUC values are reported in Table~\ref{tab:test_results}. Figure~\ref{fig:paper_roc_test} shows held-out ROC curves for all reported models, and Figure~\ref{fig:paper_pr_curves} shows the corresponding precision--recall curves.

\begin{figure}[!htbp]
    \centering
    \subfloat[ROC curves.\label{fig:paper_roc_test}]{%
        \includegraphics[width=0.49\linewidth]{figures/fig03_roc_test.png}}
    \hfill
    \subfloat[Precision--recall curves.\label{fig:paper_pr_curves}]{%
        \includegraphics[width=0.49\linewidth]{figures/fig04_pr_curves.png}}
    \caption{Receiver operating characteristic and precision--recall curves comparing all nine models on the held-out test set.}
    \label{fig:classwise_curve_analysis}
\end{figure}

\subsection{Error Analysis}
False negatives are the most safety-critical error type because they correspond to unsafe/risky hanging-travel cases predicted as safe/normal. At the balanced operating point, the ensemble detects 94.7\% of unsafe test images, missing only 2 of 38 unsafe test images. The balanced, high-recall, and max-recall thresholds are respectively 0.170, 0.175, and 0.175, all tuned on validation; they happen to produce the same rounded test result. Table~\ref{tab:operating_modes} lists the three modes, and Figure~\ref{fig:operating_analysis} shows the threshold trade-off.

\begin{table}[!htbp]
\centering
\caption{Selectable operating points for the deployed ensemble. All thresholds are tuned on validation; the displayed test metrics are evaluated afterward.}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.98\linewidth,height=0.34\textheight,keepaspectratio]{figures/fig06_tradeoff.png}
    \caption{Ensemble threshold behavior on the held-out test set: accuracy and unsafe recall against the decision threshold, with the operating points selected from validation predictions marked. Because the retrained dataset is near-balanced, the three points nearly coincide.}
    \label{fig:operating_analysis}
\end{figure}

\section{Discussion}
\subsection{Interpretation of Results}
The retrained ensemble provides a strong, recall-oriented balanced operating point, reaching 86.1\% test accuracy, 94.7\% unsafe recall, 0.867 unsafe $F_1$, 0.970 ROC-AUC, and 0.978 PR-AUC. The current run shows strong ranking performance, while the small test set warrants cautious interpretation of the estimate.
...
The deployed configuration uses the ResNet50--ConvNeXt ensemble because the final model-selection procedure fixes these two architecturally different transfer backbones before test evaluation and averages their unsafe-class probabilities. This is consistent with the safety-oriented objective of prioritizing unsafe recall while retaining acceptable discrimination. On the test set, the ensemble reaches 94.7\% unsafe recall, compared with ResNet50's 92.1\%; EfficientNet-B0 reaches 97.4\% unsafe recall but has lower balanced accuracy (83.5\%). These test-set comparisons are descriptive and were not used to select the ensemble.
...
The train-to-test gaps in Table~\ref{tab:generalization} range from 6.7 to 16.5 percentage points. Logistic Regression has the smallest gap, while ConvNeXt-Tiny has the largest. These current-run gaps show that performance differs between the training and held-out partitions; they do not, by themselves, support a before-and-after claim about reduced overfitting. Figure~\ref{fig:generalization_analysis} visualizes the per-model train, validation, and test error and the train-to-test gap.
...
\begin{figure}[!htbp]
\centering
\captionof{table}{Train, Validation, and Test Generalization}
...
\subfloat[Train, validation, and test error.\label{fig:generalization_error}]{%
    \includegraphics[width=\linewidth,height=0.32\textheight,keepaspectratio]{figures/fig07_train_val_test_error.png}}
\par\smallskip
\subfloat[Train-to-test generalization gap.\label{fig:generalization_gap}]{%
    \includegraphics[width=\linewidth,height=0.32\textheight,keepaspectratio]{figures/fig08_generalization_gap.png}}
...
The updated result set compares classical models (Logistic Regression, Gaussian Naive Bayes, and SVM with an RBF kernel) with deep models (a CNN trained from scratch, ResNet18, EfficientNet-B0, ResNet50, ConvNeXt-Tiny, and a ResNet50--ConvNeXt ensemble). On the untouched test set, the deployed ensemble reaches a recall-oriented balanced result with 86.1\% accuracy, 94.7\% unsafe recall, and 0.970 ROC-AUC, while ResNet50 gives the highest single-model accuracy at 92.4\%. Because the retrained data is near-balanced, the balanced, high-recall, and max-recall thresholds coincide at the same 86.1\%-accuracy and 94.7\%-recall operating point rather than trading accuracy for a separate perfect-recall mode.


As a practical outcome of this work, we developed an online platform that classifies public-transport images as safe/normal or unsafe/risky. The project resources are publicly accessible through the \href{https://github.com/TanzilaKhan1/Detection-of-Hanging-Passengers/}{source-code repository}, \href{https://data.mendeley.com/datasets/24n4zpdt69/1}{Mendeley Data}, and the \href{https://ml-predictor-app.streamlit.app/}{deployed web platform}.
...
\url{https://www.sciencedirect.com/science/article/abs/pii/S0030402619315736}
...
\href{https://doi.org/10.1061/9780784413210.016}{doi: 10.1061/9780784413210.016}
...
\href{https://doi.org/10.5038/2375-0901.16.2.6}{doi: 10.5038/2375-0901.16.2.6}
...
\url{https://www.mdpi.com/2079-9292/13/13/2579}
...
\href{https://doi.org/10.3390/s26051418}{doi: 10.3390/s26051418}
...
\href{https://doi.org/10.1109/ITSC55140.2022.9921801}{doi: 10.1109/ITSC55140.2022.9921801}
...
\href{https://doi.org/10.1109/ACCESS.2024.3365138}{doi: 10.1109/ACCESS.2024.3365138}
...
\href{https://doi.org/10.1038/s41598-025-85962-8}{doi: 10.1038/s41598-025-85962-8}
...
\href{https://doi.org/10.3390/app15105367}{doi: 10.3390/app15105367}
...
\href{https://doi.org/10.1109/TITS.2008.922938}{doi: 10.1109/TITS.2008.922938}
...
\href{https://doi.org/10.1109/TCSVT.2012.2203741}{doi: 10.1109/TCSVT.2012.2203741}
...
\href{https://arxiv.org/abs/2312.10701}{arXiv:2312.10701}
...
\href{https://doi.org/10.1016/j.dib.2020.106465}{doi: 10.1016/j.dib.2020.106465}
...
\href{https://doi.org/10.1109/ICCCNT61001.2024.10725076}{doi: 10.1109/ICCCNT61001.2024.10725076}
...
S. van der Walt, J. L. Sch\"onberger, J. N\'u\~nez-Iglesias, F. Boulogne, J. D. Warner, N. Yager, E. Gouillart, and T. Yu,
...
\href{https://arxiv.org/abs/2201.03545}{arXiv:2201.03545}
...
\href{https://arxiv.org/abs/1502.03167}{arXiv:1502.03167}
...
\href{https://doi.org/10.1186/s12864-019-6413-7}{doi: 10.1186/s12864-019-6413-7}
